...
while($job = $queue->reserve()) {
    ...
    {
        case 1:
            $scanner = new \Library\NewAgent($pdo, $received['profile_id'], $received['scan_name'], $received['ip_address']);
            $result = $scanner->deployAgent();
            $worker->checkSuccess($result, $queue, $job);
            break;
        case 2:
            $scanner = new \Library\ExistingAgent($received['profile_id'], $pdo, $received['host_id']);
