...
class Hosts extends ReconAbstract{
    ...
    public function addHost($postData)
    {
        ...
        $hostsQuery = $this->getPdo()->prepare('INSERT INTO hosts (host_name, ip_address, type, tracker, status) VALUES(? ,? ,?, ?)');
        $response = $hostsQuery->execute(array($postData['host_name'], $postData['ip_address'], $postData['type'], $tracker, $postData['status']));
