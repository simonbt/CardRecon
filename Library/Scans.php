...
class Scans extends ReconAbstract{
    ...
    {
        $query = $this->getPdo()->prepare('SELECT hosts.*, host_status.status FROM hosts LEFT JOIN host_status on host_status.id = hosts.status WHERE NOT hosts.status =4 AND NOT hosts.status=99');
        $query->execute();
